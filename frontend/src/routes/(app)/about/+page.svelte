<svelte:head>
	<title>About Us | The Grey Skies</title>
</svelte:head>

<img src="https://cdn.greysdawn.com/img/69f5.png" class="hero" />
<h1>Hi, we're the <span style="color: #aaa">Grey Skies</span></h1>
<p>
	We're a <a href="/flags/traumagenic">traumagenic</a>
	and <a href="/flags/polyfrag">polyfragmented</a> system of
	over 100 known headmates. If you're unsure what that means,
	that's okay{"-"} just call us <b style="color: #aaa">Grey</b>!
</p>

<h2>We make stuff</h2>
<p>
	We love to create! Over the years we've made various things,
	from bots to websites to art. Much of that can be found on this site.
	<br/>
	At the moment, our primary focus is on our Discord bots
	and the websites that surround them.
	We also make other things from time to time, though, so keep
	an eye on on our blog for when we make releases!
</p>

<h2>Places you can find us</h2>
<p>
	We're mostly available on Discord @greysdawn, but
	we also have some accounts in other places:
	<br/>
	<a href="https://discord.gg/3y228CvJyQ">Discord content server</a>
	<br/>
	<a href="https://discord.gg/EvDmXGt">Bot support server</a>
	<br/>
	<a href="https://bsky.app/profile/greysdawn.com">BlueSky</a>
	<br/>
	<a href="https://twitter.com/greysdawn">Twitter/X</a>
	<br/>
	<a href="https://ko-fi.com/greysdawn">Ko-Fi</a>
	<br/>
	<a href="https://patreon.com/greysdawn">Patreon</a>
</p>

<style>
	.hero {
		width: 90%;
		height: auto;
		margin-top: 50px;
		border-radius: 20px;
		max-width: 700px;
	}

	p {
		font-size: 1.2rem;
		width: 90%;
		max-width: 700px;
	}

	h1 {
		font-size: 40px;
		max-width: 700px;
		width: 90%;
	}

	h2, h3 {
		width: 90%;
		max-width: 700px;
	}
</style>